(function () {
  ...
  EFDRCE.handlePaste = function (e) {        
    if (!EFDRCE.CONF.process_paste) {
      // e.preventDefault()
      // window.pycmd('EFDRCE!pasteNOEXT')
      return
    }
  ...
  // // Function to clear the selection from all tags
  // function clearAllFormatting(el) {            
  //   EFDRCE.saveUndoState(el)    
  //   const selection = window.getSelection();
  //   if (!selection || selection.isCollapsed) return;
    
  //   // window.pycmd('EFDRCE!clearAllFormatting')
  //   // return

  //   // Hidden for now, not the most successful algorithm
  ...
  function clearAllFormatting(el) {            
    ...
    // If the text is selected -difficult deletion
    ...
    //If the cursor is just standing, delete the current element
    else {
         const node = selection.anchorNode;        
        // Finding the parent element (not the text node)
        let elementToRemove = node.nodeType === Node.TEXT_NODE ? node.parentNode : node;        
        // Make sure it's an element and not document or body
        if (elementToRemove && elementToRemove.nodeType === Node.ELEMENT_NODE && 
            elementToRemove !== document.body && elementToRemove !== document.documentElement) {            
            // Save the text
            const text = elementToRemove.textContent || elementToRemove.innerText || '';            
            // Create a text node
  ...
  const registerFormattingShortcut = function () {
    for (const key in EFDRCE.CONF.special_formatting) {
      ...
      registerShortcut(shortcut, (event, el) => {
        if (format.arg) {
            let valueToInsert = format.arg.value;
            
            if (format.command === "insertHTML" && valueToInsert.includes("$$")) {
                const selText = window.getSelection().toString();
                const parts = valueToInsert.split("$$");
                // we form new HTML: before $$ + selected text + after $$
                valueToInsert = (parts[0] || "") + selText + (parts[1] || "");
                document.execCommand(format.command, false, valueToInsert);
            }
            else {
                document.execCommand(format.command, false, format.arg.value)
            }
        } else {
          if (format.command === "removeFormatALL") {
              event.preventDefault();
              clearAllFormatting(el);              
          }
          else {
  ...
  EFDRCE.registerConfig = function (confStr) {
    ...
    // translations in the form q()
    window.EFDRCE.initTranslations(EFDRCE.CONF);
  }
  ...
  EFDRCE.setupReviewer = function () {
    ...
    registerShortcut(EFDRCE.CONF.shortcuts['toggles_process_paste'], (event, el) => {    
      event.preventDefault()
      EFDRCE.CONF.process_paste = !EFDRCE.CONF.process_paste
      if(EFDRCE.CONF.process_paste) {
        window.FshowToast(q("q_process_paste_true", 'HTML processing enabled'))  
      }
      else {
        window.FshowToast(q("q_process_paste_false", 'No HTML processing - paste everything as is')) 
      }

      elqa = document.getElementById('qa')
      if(elqa) {
        if(EFDRCE.CONF.process_paste) {
          elqa.classList.remove('past-all-html')
        }
        else {
          elqa.classList.add('past-all-html')
        }
      }
    })
...
/*
  to be able to receive translations
*/
  (function() {
    ...
    // Initialization (called once when loading the config)
    function initTranslations(config) {
      if (isInitialized) return;
      
      qConfig = config?.q || {};

      for (const key in qConfig) {
        const value = qConfig[key];
        translationsCache[key] = b64DecodeUnicode(String(value))      
      }      
      
      isInitialized = true;
    }
    
    // Ultra-optimized q function
    function q(key, defaultValue = "") {
      const cleanKey = key.trim();
      
      // Quick cache check
      const cached = translationsCache[cleanKey];
      ...
      // Checking the config
      const fromConfig = qConfig[cleanKey];
      
      if (fromConfig !== undefined) {
        // Caching for future calls
        translationsCache[cleanKey] = fromConfig;
        return fromConfig;
      }
      
      // Define defaultValue if necessary
      if (defaultValue === "" && cleanKey !== "") {
        defaultValue = cleanKey;
      }
      
      // We also cache defaultValue to avoid repeated checks
      translationsCache[cleanKey] = defaultValue;
      
      return defaultValue;
    }
    
    // We export to the global area
    window.EFDRCE = window.EFDRCE || {};
    window.EFDRCE.q = q;
    window.EFDRCE.initTranslations = initTranslations;
