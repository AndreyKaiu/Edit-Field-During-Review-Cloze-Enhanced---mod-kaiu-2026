...
from aqt import mw
from aqt.qt import *
from aqt.qt import QTextBrowser, QWidget, QVBoxLayout, QHBoxLayout, QScrollArea
...
from .reviewer import overlay_action, toggle_cloze_overlay


conf = ConfigManager()

def help_tab(conf_window: ConfigWindow) -> None:
    tab = conf_window.add_tab(q("q_Help"))
    ...
    browser.setHtml(firstrun.tutorial)
    browser.document().setDocumentMargin(10)
    tab.addWidget(browser)

_cloze_tools_auto_show_overlay = None
_cloze_tools_show_HTML_in_overlay = None

def general_tab(conf_window: ConfigWindow) -> None:
    global _cloze_tools_auto_show_overlay, _cloze_tools_show_HTML_in_overlay
    tab = conf_window.add_tab(q("q_General"))
    tab.checkbox(
        "ctrl_click",
        q("q_Ctrl_Click_to_edit_field"),
        tooltip=q("q_If_not_checked_there_is"),
    )
    tab.checkbox("outline", q("q_Show_a_blue_outline"))
    tab.checkbox("process_paste",
                 q("q_Process_pasted_content_for_images_and_HTML"),
                 tooltip=f"{q('q_If_you_disable_this_feature_the_entire_HTML_code')} {conf.get('toggles_process_paste', 'Alt + V')}",
    )

    _cloze_tools_auto_show_overlay = tab.checkbox("cloze_tools.auto_show_overlay", q("q_Show_Cloze_Info_Overlay"))
    _cloze_tools_auto_show_overlay.stateChanged.connect(
        lambda state: overlay_action.setChecked(state)
    )

    # Create a horizontal layout for two checkboxes
    hbox_widget = QWidget()
    hbox_layout = QHBoxLayout()
    hbox_layout.setContentsMargins(0, 0, 0, 0)
    cb1 = tab.text("    ")
    hbox_layout.addWidget(cb1)
    _cloze_tools_show_HTML_in_overlay = tab.checkbox("cloze_tools.show_HTML_in_overlay", q("q_show_HTML_in_overlay"))    
    hbox_layout.addWidget(_cloze_tools_show_HTML_in_overlay)
    hbox_layout.addStretch()
    hbox_widget.setLayout(hbox_layout)
    tab.addWidget(hbox_widget)    

    tab.checkbox("disable_autoplay_after_edit", q("q_Disable_Autoplay_after_edit"))
    tag_options = ["div", "span"]
    tab.dropdown(
        "tag",
        tag_options,
        tag_options,
        q("q_HTML_tag_to_use_for_editable_field"),
        tooltip=q("q_div_is_recommended"),
    )
    tab.text_input(
        "shortcuts.cloze-alt",
    ...
    tab.checkbox(
        ...
        q("q_Use_image_resizing"),
        tooltip=q("q_Even_when_unchecked_you_can_toggle"),
    )
    option_labels = [
        "Don't preserve ratio",
        q("q_Preserve_ratio_when_using_corner"),
        q("q_Always_preserve_ratio"),
    ]
    option_values = [0, 1, 2]
    tab.dropdown(
        ...
        option_labels,
        option_values,
        q("q_Image_resizing_mode"),
    )
    tab.text_input(
        "shortcuts.image-resize",
        q("q_Shortcut_for_image_resize_mode"),
        tooltip=q("q_Pressing_this_shortcut_toggles_the_image_resize_mode"),
    )
    tab.stretch()
...
def formatting_tab(conf_window: ConfigWindow) -> None:
    ...
    tab = conf_window.add_tab(q("q_Formatting"))
    ...
    for formatting in conf["special_formatting"]:
        ...
        if conf[f"{item_key}.arg"] is not None:
            if conf[f"{item_key}.arg.type"] == "color":
                ...
                hlayout.text_input(f"{item_key}.arg.value").setFixedWidth(160)
        hlayout.stretch()
    tab.stretch()
...
def fields_tab(conf_window: ConfigWindow) -> None:
    tab = conf_window.add_tab(q("q_Fields"))
    dropdown = QComboBox()
    tab.addWidget(dropdown)
    tab.space(10)
    tab.text(q("q_Check_the_boxes_to_make_the_fields_editable_while_reviewing"))
    qlist = QListWidget()
    ...
    button = QPushButton(q("q_Make_every_field_in_every_note_type_editable"))
...
<style>
    ...
    li { margin: 4px 0; }
</style>""" + f"""<h2>{firstrun.addon_name}<br>{q("q_Version")}: {firstrun.version_string}</h2>{q("q_about_text")}"""


from aqt.qt import QTextBrowser, Qt

def about_tab(conf_window: ConfigWindow) -> None:
    tab = conf_window.add_tab(q("q_About"))
...
def with_window(conf_window: ConfigWindow) -> None:
    global _cloze_tools_auto_show_overlay, _cloze_tools_show_HTML_in_overlay  
    conf_window.set_footer(q("q_Changes_will_take_effect_when_you_start_a_review_session"))    
    conf.load()    
    if _cloze_tools_auto_show_overlay:
        value = conf.get("cloze_tools.auto_show_overlay")
        _cloze_tools_auto_show_overlay.setChecked(value)        
    if _cloze_tools_show_HTML_in_overlay:
        value = conf.get("cloze_tools.show_HTML_in_overlay")
        _cloze_tools_show_HTML_in_overlay.setChecked(value)        
    
conf.use_custom_window()
conf.on_window_open(with_window)
conf.add_config_tab(help_tab)
