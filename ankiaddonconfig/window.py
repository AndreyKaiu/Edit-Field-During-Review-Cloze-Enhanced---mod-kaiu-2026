...
class ConfigWindow(QDialog):
    ...
    def setup_buttons(self, btn_box: "ConfigLayout") -> None:
        self.advanced_btn = QPushButton(q("q_Advanced"))
        self.advanced_btn.clicked.connect(self.on_advanced)
        btn_box.addWidget(self.advanced_btn)

        self.reset_btn = QPushButton(q("q_Restore_Defaults"))
        self.reset_btn.clicked.connect(self.on_reset)
        btn_box.addWidget(self.reset_btn)

        btn_box.addStretch(1)

        self.cancel_btn = QPushButton(q("q_Cancel"))
        self.cancel_btn.clicked.connect(self.on_cancel)
        btn_box.addWidget(self.cancel_btn)

        self.save_btn = QPushButton(q("q_Save"))
    ...
    def update_widgets(self) -> None:
        try:
            ...
            dial, bbox = showText(
                q("q_Invalid_Config_Please_fix") + " \n\n"
                + str(e),
                title=q("q_Invalid_Config"),
                parent=advanced,
                run=False,
            )
            button = QPushButton(q("q_Quit_Config"))
    ...
    def on_reset(self) -> None:
        ...
        self.update_widgets()
        tooltip(q("q_Press_save_to_save_changes"))
...
class ConfigLayout(QBoxLayout):
        ...
        def update() -> None:
            ...
            try:
                ...
                raise InvalidConfigValueError(
                    key, q("q_any_value_in_list") + " " + str(values), val
                )
    ...
    def shortcut_input(
        ...
        shortcut_clear_btn = QPushButton(q("q_Clear"))
